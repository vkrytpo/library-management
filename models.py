...
from django_prices.models import MoneyField, TaxedMoney
from jsonfield import JSONField
from prices import sum as sum_prices, Money
...
class Cart(models.Model):
    ...
    def get_total_without_discount(self, discounts=None, taxes=None):
        """Return the total cost of the cart prior to shipping."""
        subtotals = [
            line.get_total_without_discount() for line in self.lines.all()]
        if not subtotals:
            raise AttributeError('Calling get_total() on an empty cart')
        return sum_prices(subtotals)
    ...
    def add(self, variant, quantity=1, data=None, replace=False,
            check_quantity=True, cid=None, variant_price=100):
        ...
        # dummy_data = ''
        if quantity == 0:
            cart_line = self.lines.get(id=cid)
        else:
            cart_line, dummy_data = self.lines.get_or_create(
                variant=variant, quantity=quantity, data=data, id=cid, variant_price=variant_price)
        if replace:
            new_quantity = quantity
        else:
            new_quantity = cart_line.quantity + quantity-1
        if new_quantity < 0:
...
class CartLine(models.Model):
    ...
    variant_price = MoneyField(
        currency=settings.DEFAULT_CURRENCY, max_digits=12,
        decimal_places=settings.DEFAULT_DECIMAL_PLACES ,default=None)
    ...
    def __setstate__(self, data, variant_price):
        self.variant, self.quantity, self.data = data, self.variant_price = variant_price

    def get_total(self, discounts=None, taxes=None):
        """Return the total price of this line."""
        amount = self.quantity * TaxedMoney(self.variant_price,self.variant_price)
        return amount.quantize(CENTS)

    def get_total_without_discount(self):
        amount = self.quantity * TaxedMoney(Money(self.data['total_price_without_discount'], settings.DEFAULT_CURRENCY),
                                            Money(self.data['total_price_without_discount'], settings.DEFAULT_CURRENCY))
        return amount.quantize(CENTS)
